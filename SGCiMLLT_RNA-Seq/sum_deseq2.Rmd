...
# not necessary as separate metadata files for each cell line
...
dt <- filter_genes(as.data.frame(res_24H), name="SUM_DMSOvSGC_24H")
...
```{r, message=FALSE}
dt <- filter_genes(as.data.frame(res_7D), name="SUM_DMSOvSGC_7D")
...
res_24h <- read.csv("results/SUM_DMSOvSGC_24H_res.csv") 
sig_24h <- res_24h[res_24h$padj<0.05 ,]

res_7d <- read.csv("results/SUM_DMSOvSGC_7D_res.csv") 
...
png("venn/SUM_downregulated_venn.png")
vennDiagram(a, circle.col = wes_palette("Darjeeling1"))

down <- merge_down[complete.cases(merge_down),]

write.csv(down, file="venn/SUM_down.csv")

```

![](venn/SUM_downregulated_venn.png)
...
library(wesanderson)
c("red", "blue", "darkgreen", "yellow")
png("venn/SUM_upregulated_venn.png")
vennDiagram(a, circle.col = wes_palette("Darjeeling1"))

up <- merge_up[complete.cases(merge_up),]

write.csv(up, file="venn/SUM_up.csv")

```
![](venn/SUM_upregulated_venn.png)
