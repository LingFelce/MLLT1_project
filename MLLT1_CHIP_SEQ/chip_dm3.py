...
@follows(mkdir('dm3_bam'))
@collate('*.fastq.gz', regex(r'(.*)_R[1-2].fastq.gz'), r'dm3_bam/\1.bam')
def align_reads2(infiles, outfile):
    ...
    cmd = '''(bowtie2 -x %(bowtie2_ref)s -1 %(fq1)s -2 %(fq2)s -p %(threads)s) 2>> dm3_stats.txt  |
