...
def trimmomatic(infile, outfile, phred, trimmomatic_option):
    output_prefix = "processed/" + infile.replace(".fastq.gz", "")
    statement = """trimmomatic PE -threads %(job_threads)s %(phred)s %(infile)s %(outfile)s%(trimmomatic_option)s 2>> %(output_prefix)s.log""" 
    P.run(statement, job_queue=P.PARAMS['queue'], job_memory ='8G')
...
#output will be abundance.h5, abundance.tsv and run_info.json (run info). Can import .h5 into R and use R tximport package to convert to counts
