...
## MLLT1 ChIP-Seq and CRISPR + SGCiMLLT RNA-Seq metaplots
Generating metaplot to visualise changes in distribution of MLLT1 over peaks when treated with SGC-iMLLT compared to DMSO
...
```{r}
# read in files
sum_24h_down <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/SUM_MLLT1_24H_down_degs.txt", sep="\t")
  
sum_24h_up <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/SUM_MLLT1_24H_up_degs.txt", sep="\t")

sum_7d_down <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/SUM_MLLT1_7D_down_degs.txt", sep="\t")

sum_7d_up <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/SUM_MLLT1_7D_up_degs.txt", sep="\t")

mcf_24h_down <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/MCF_MLLT1_24H_down_degs.txt", sep="\t")

mcf_24h_up <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/MCF_MLLT1_24H_up_degs.txt", sep="\t")

mcf_7d_down <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/MCF_MLLT1_7D_down_degs.txt", sep="\t")

mcf_7d_up <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/MCF_MLLT1_7D_up_degs.txt", sep="\t")
...
p1 <- ggplot(list[[1]], aes(x=position, y=count, color=treatment)) +
  geom_line(size=2, aes(linetype=treatment)) +
  labs(title="MLLT1 distribution at MLLT1 upregulated genes in SUM159 with 24 hours treatment",
       x="Position relative to peak centre (bp)", 
       y = "Read count (per 10 million reads") +
  scale_color_manual(values=c("grey", "#55A0FF")) + theme_classic() +
  scale_linetype_manual(values = c("dashed", "solid")) + 
...
p2 <- ggplot(list[[2]], aes(x=position, y=count, color=treatment)) +
  geom_line(size=2, aes(linetype=treatment)) +
  labs(title="MLLT1 distribution at MLLT1 downregulated genes in SUM159 with 24 hours treatment",
       x="Position relative to peak centre (bp)", 
       y = "Read count (per 10 million reads") +
  scale_color_manual(values=c("grey", "#55A0FF")) + theme_classic() +
  scale_linetype_manual(values = c("dashed", "solid")) + 
...
p3 <- ggplot(list[[3]], aes(x=position, y=count, color=treatment)) +
  geom_line(size=2, aes(linetype=treatment)) +
  labs(title="MLLT1 distribution at MLLT1 upregulated genes in SUM159 with 7 days treatment",
       x="Position relative to peak centre (bp)", 
       y = "Read count (per 10 million reads") +
  scale_color_manual(values=c("grey", "#55A0FF")) + theme_classic() +
  scale_linetype_manual(values = c("dashed", "solid")) + 
...
p4 <- ggplot(list[[4]], aes(x=position, y=count, color=treatment)) +
  geom_line(size=2, aes(linetype=treatment)) +
  labs(title="MLLT1 distribution at MLLT1 downregulated genes in SUM159 with 7 days treatment",
       x="Position relative to peak centre (bp)", 
       y = "Read count (per 10 million reads") +
  scale_color_manual(values=c("grey", "#55A0FF")) + theme_classic() +
  scale_linetype_manual(values = c("dashed", "solid")) + 
...
p5 <- ggplot(list[[5]], aes(x=position, y=count, color=treatment)) +
  geom_line(size=2, aes(linetype=treatment)) +
  labs(title="MLLT1 distribution at MLLT1 upregulated genes in MCF-7 with 24 hours treatment",
       x="Position relative to peak centre (bp)", 
       y = "Read count (per 10 million reads") +
  scale_color_manual(values=c("grey", "#55A0FF")) + theme_classic() +
  scale_linetype_manual(values = c("dashed", "solid")) + 
...
p6 <- ggplot(list[[6]], aes(x=position, y=count, color=treatment)) +
  geom_line(size=2, aes(linetype=treatment)) +
  labs(title="MLLT1 distribution at MLLT1 downregulated genes in MCF-7 with 24 hours treatment",
       x="Position relative to peak centre (bp)", 
       y = "Read count (per 10 million reads") +
  scale_color_manual(values=c("grey", "#55A0FF")) + theme_classic() +
  scale_linetype_manual(values = c("dashed", "solid")) + 
...
p7 <- ggplot(list[[7]], aes(x=position, y=count, color=treatment)) +
  geom_line(size=2, aes(linetype=treatment)) +
  labs(title="MLLT1 distribution at MLLT1 upregulated genes in MCF-7 with 7 days treatment",
       x="Position relative to peak centre (bp)", 
       y = "Read count (per 10 million reads") +
  scale_color_manual(values=c("grey", "#55A0FF")) + theme_classic() +
  scale_linetype_manual(values = c("dashed", "solid")) + 
...
p8 <- ggplot(list[[8]], aes(x=position, y=count, color=treatment)) +
  geom_line(size=2, aes(linetype=treatment)) +
  labs(title="MLLT1 distribution at MLLT1 downregulated genes in MCF-7 with 7 days treatment",
       x="Position relative to peak centre (bp)", 
       y = "Read count (per 10 million reads") +
  scale_color_manual(values=c("grey", "#55A0FF")) + theme_classic() +
  scale_linetype_manual(values = c("dashed", "solid")) + 
...
```{r}
# read in files
sum_24h_down <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/SUM_MLLT1_24H_down_degs_bp.txt", sep="\t")
  
sum_24h_up <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/SUM_MLLT1_24H_up_degs_bp.txt", sep="\t")

sum_7d_down <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/SUM_MLLT1_7D_down_degs_bp.txt", sep="\t")

sum_7d_up <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/SUM_MLLT1_7D_up_degs_bp.txt", sep="\t")

mcf_24h_down <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/MCF_MLLT1_24H_down_degs_bp.txt", sep="\t")

mcf_24h_up <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/MCF_MLLT1_24H_up_degs_bp.txt", sep="\t")

mcf_7d_down <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/MCF_MLLT1_7D_down_degs_bp.txt", sep="\t")

mcf_7d_up <- read.delim("/stopgap/donglab/ling/t1-data/MLLT1/ChIP-Seq/analyses/results/MCF_MLLT1_7D_up_degs_bp.txt", sep="\t")
