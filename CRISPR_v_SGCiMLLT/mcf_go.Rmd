...
Genes filtered so only do GO analysis for those with log fold change less than -0.5 (otherwise too many non-specific GO terms)
...
# Filter genes with -0.2 log2 fold change
genes_24h_filter <- genes_24h %>%
  filter(crispr_1_LFC < -0.2)
genes_24h_filter <- genes_24h_filter %>%
  filter(crispr_2_LFC < -0.2)
genes_24h_filter <- genes_24h_filter %>%
  filter(run2_24h_LFC < -0.2)
genes_24h_filter <- genes_24h_filter %>%
  filter(merge_24h_LFC < -0.2)
...
Used all 717 genes for GO analysis, as when selected only genes with log fold change larger than 0.5, didn't get any GO terms.
...
```{r}
library(readr)
genes_7d <- read_csv("mcf_shared_genes_7d_down.csv")
genes_7d[1] <- NULL
...
# Filter genes with -0.1 log2 fold change
genes_7d_filter <- genes_7d %>%
  filter(crispr_1_LFC < -0.1)
genes_7d_filter <- genes_7d_filter %>%
  filter(crispr_2_LFC < -0.1)
genes_7d_filter <- genes_7d_filter %>%
  filter(run2_7d_LFC < -0.1)
genes_7d_filter <- genes_7d_filter %>%
  filter(merge_7d_LFC < -0.1)
...
```{r}
# Change Entrez IDs from numbers to characters
geneset <- as.character(genes_7d_filter$Entrez.Gene)
...
```{r, fig.height=12, fig.width=14}
# logFC is optional, but if available will colour-code the nodes
logFC <- genes_7d_filter$crispr_1_LFC
# set name of object
names(logFC) <- genes_7d_filter$Entrez.Gene
cnetplot(ego, foldChange = logFC, showCategory = 5, circular = FALSE)
# show 5 GO terms, if set circular to TRUE then GO terms won't be in middle, will be on edge of circle
# size = number of genes, fold change = red for upregulated, green for downregulated
```
...
```{r}
library(readr)
genes_7d <- read_csv("mcf_shared_genes_7d_up.csv")
genes_7d[1] <- NULL
...
# Filter genes with 0.25 log2 fold change
genes_7d_filter <- genes_7d %>%
  filter(crispr_1_LFC > 0.25)
genes_7d_filter <- genes_7d_filter %>%
  filter(merge_7d_LFC > 0.25)
genes_7d_filter <- genes_7d_filter %>%
  filter(crispr_2_LFC > 0.25)
genes_7d_filter <- genes_7d_filter %>%
  filter(run2_7d_LFC > 0.25)
